...
pdf() {
    ...
    local combined_markdown="_temp/chapters.md"
    local intermediate_tex="_temp/intermediate.tex"
    local final_pdf="$output_directory/$output_filename.pdf"

    # --- Generate Intermediate LaTeX File ---
    echo "Generating intermediate LaTeX file ($intermediate_tex)..."
    $pandoc_command "$combined_markdown" \
        --defaults settings/pdf.yml \
        --to latex \
        --standalone \
        --output "$intermediate_tex" \
        --verbose

    # Check if the .tex file was created successfully
    if [ ! -f "$intermediate_tex" ]; then
        echo "❌ Failed to generate intermediate LaTeX file."
        # Optionally decide if you want to stop here or still attempt PDF generation
        # exit 1 # Uncomment to stop if .tex generation fails
    else
        echo "✅ Intermediate LaTeX file saved to $intermediate_tex"
        echo "-> If running in GHA, ensure this file is uploaded as an artifact."
    fi

    # --- Attempt Original PDF Generation (Optional but recommended for comparison) ---
    # You might still want to run the original command to see the LaTeX error happen in GHA logs
    # Or, you could try compiling the intermediate.tex file directly using latexmk/pdflatex
    # Sticking to the original command for now:
    echo "Attempting direct PDF generation ($final_pdf)..."
    $pandoc_command "$combined_markdown" \
        --defaults settings/pdf.yml \
        --output "$final_pdf" \
        --verbose

    # Report based on PDF existence (original command's success)
    if [ -f "$final_pdf" ]; then
        echo "✅ PDF generation successful."
        echo "📖 The PDF edition is now available in the $output_directory folder"
    else
        echo "❌ PDF generation failed (as expected perhaps). Check logs and the intermediate.tex artifact."
        # Consider exiting with an error code if PDF failed in GHA
        # exit 1 # Uncomment if PDF failure should fail the GHA step
    fi
}
